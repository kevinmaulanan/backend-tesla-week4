...
def populate_books(books):
    ...
    for book in books:
        try:
            ...
            # Insert to books
            sql = "INSERT INTO books (book_name, description, book_image, amazon_url, id_author, id_global_rating) VALUES (%s, %s, %s, %s, %s, %s)"
            val = (book['book_title'], book['description'], book['book_img'], str(book['amazon_url']), author_id, global_book_rating_id)
            mycursor.execute(sql, val)
